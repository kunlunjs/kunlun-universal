...
const ruanyifeng = 'https://raw.githubusercontent.com/ruanyf/weekly/master/'
...
export const crawler = async () => {
  if (!fs.existsSync(weeklyFile)) {
    ...
    const lines = (await supergent.get(ruanyifeng + 'README.md')).text.split(
      '\n'
    )
    for (const line of lines) {
      const doc = line.match(/\[(\S+)\]\(([\w\d-/.]+\.md)\)$/)
      if (doc) {
        const [_, title, mdPath] = doc
        const mdFullPath = `${ruanyifeng}${mdPath}`
        let mdContent = (await supergent.get(mdFullPath)).text
